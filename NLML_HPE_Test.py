...
def compute_interval_mae(true_angles, pred_angles_list, labels):
    true_angles = np.array(true_angles)
    
    yaw_intervals = [(-51, -33.33), (-33.33, -16.67), (-16.67, 0), (0, 16.67), (16.67, 33.33), (33.33, 51)]
    pitch_intervals = [(-41, -26.67), (-26.67, -13.33), (-13.33, 0), (0, 13.33), (13.33, 26.67), (26.67, 41)]
    roll_intervals = [(-31, -20), (-20, -10), (-10, 0), (0, 10), (10, 20), (20, 31)]
...
dataset = 'facescape' # facescape / biwi / AFLW2000
...
######## BIWI val set ##############
    ...
    for i in range(len(images)):
        ...
        if not (-50 <= true_yaw <= 50 and -40 <= true_pitch <= 40 and -30 <= true_roll <= 30):
            continue
...
elif dataset == 'facescape': # not use biwi    
...
######## Facescape val set ##############
    ...
    image_pattern = re.compile(r'ID(\d+)_\((-?\d+)_(\-?\d+)_(\-?\d+)\)\.png')
...
######## AFLW2000 val set ##############    
    ...
    pose_dataset = datasets.getDataset(dataset, val_set_path, filename_list, transformations=None, train_mode=False)
    ...
    for i, (images, r_label, cont_labels, name) in enumerate(val_loader):
        ...
        if not (-50 <= true_yaw.item() <= 50 and -40 <= true_pitch.item() <= 40 and -30 <= true_roll.item() <= 30):
            continue        
...
labels = ["NLML_HPE"]
mae_results = compute_interval_mae(true_angles, pred_angles_list, labels)
# print(mae_results)
